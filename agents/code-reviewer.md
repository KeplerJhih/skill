...
`SendMessage`、`TaskList`、`TaskCreate`、`TaskUpdate`、`TaskGet` 是 deferred tool，預設不在你的工具表裡（即使 frontmatter 已列入也要透過 `ToolSearch` 把 schema 拉進來才能呼叫）。**任何工作開始前**先執行：

```
ToolSearch query="select:SendMessage,TaskList,TaskCreate,TaskUpdate,TaskGet"
```

審查發現需要派回隊友時就用 `SendMessage`，不要靠 Lead 中轉。
