...
1) 載入名稱含 karpathy-guidelines 的 skill（以當前 available skills 清單實際名稱為準，plugin 安裝時帶命名空間前綴）
...
2) Read("~/.claude/shared/workflow-base.md")
3) 載入名稱含 karpathy-guidelines 的 skill（以當前 available skills 清單實際名稱為準，plugin 安裝時帶命名空間前綴），再依任務內容匹配載入專屬 Skill（情境 A 可延後至取得需求後補做）
