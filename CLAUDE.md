...
- **標準流程入口**：`/doit`（單人 Tech Lead）、`/team`（多 Agent 團隊）。兩者共用 `~/.claude/shared/workflow-base.md`。
- **新專案 bootstrap**：在專案根目錄執行 `bash ~/.claude/script/init.sh`（複製 run.sh / sleep.sh / CLAUDE.md 範本，只補缺不覆蓋；加 `--mcp` 另複製根目錄 `.mcp.json`）。
