...
此技能專注於 React 前端工程 (Frontend Engineering) 核心原則，確保 React 應用程式在各類裝置（手機、平板、桌面）上的品質、效能與一致性。適用於任何專案；下述技術堆疊與目錄路徑為**預設慣例**，一律以專案 `package.json` 與 CLAUDE.md 實際偵測為準。

## 🛠️ 預設技術堆疊（以專案 package.json 偵測為準）

- **React** + **TypeScript**
- **Vite**（開發伺服器預設 port 5173）
- **React Router**（前端路由）
- **Tailwind CSS**（樣式）
...
> 版本與實際元件庫依專案 `package.json` 為準，不假設特定大版本行為；專案採 Ant Design / shadcn 等其他組合時，以專案為準。
...
- **禁止事項**：不可在元件中使用 `import.meta.env.VITE_API_BASE` 或任何硬編碼的 `localhost:PORT` 拼接 URL（專案 CLAUDE.md 另有約定除外）。
...
1. **代碼質量**：
   - 在前端目錄（預設 `frontend/main`，依專案 CLAUDE.md）執行 `make lint` 且無錯誤。
   - 無未使用的變數或 `any` 型別。
2. **RWD 跑版檢查（必做）**：
