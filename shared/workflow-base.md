...
## 📍 現況確認（藍圖前 MANDATORY，Code / Config）

> **目的**：防止為「可能發生的問題」規劃解法。需求是「支援 X」「處理 Y 情境」「優化 Z」時，先確認問題真的存在、影響多大，再談怎麼做。
> 曾經的雷：規劃了完整的格式轉檔方案，實作前才發現主要裝置上傳的根本不是那種格式，資料庫裡一筆都沒有。

藍圖的「📍 現況」段必答三問，**用資料回答，不用推測**：

1. **誰會遇到**：哪些使用者 / 裝置 / 入口
2. **什麼情況下**：實機行為、資料庫實際數字（查得到就查，一分鐘的事）
3. **多常**：次數或比例；查不到就寫「查不到，假設 …」讓用戶看得到假設

補充規則：

- **對照資源上限**：會吃記憶體 / CPU / 儲存的處理（影像、影片、大檔、批次），先查部署的 limits（Pod resources、機器規格），寫進風險點
- **實測關卡**：有技術不確定的項目（新套件、效能、相容性），執行步驟第一步設「實測，不達標就停下回報」，不硬做
- Bug 修復類任務這段就是「已重現：…」一句話，不會成為負擔
...
| 決策點超過三個、或跨多層（資料表 / API / 前端 / 部署）的藍圖 | 以 `~/.claude/shared/templates/blueprint.html` 為底產 HTML 藍圖（Artifact 或本地檔）：決策卡列選項並標建議、現況 / 流程 / 變更 / 驗證 / 步驟分段；聊天只放摘要與待決策點，以「回 Y 全照建議」收斂 |
