...
| 變數 | 值範例 | 用途 |
|------|--------|------|
| `{BACKEND_SKILL}` | `backend-go` 或 `backend-python` | 後端 Agent 載入的 Skill |
...
在分析計畫後、啟動 Agent 前，執行 MCP 動態偵測：

1. **列舉可用 MCP**：從 system-reminder 中的 deferred tools 列表提取 `mcp__<server-name>__` 前綴，去重得到當前可用的 MCP server 清單
2. **任務匹配**：根據計畫內容與技術棧，篩選出與本次任務相關的 MCP 工具
3. **分配給 Agent**：在每個 Agent 的 prompt 中加入其可使用的 MCP 工具清單
...
1. **角色**：前端工程師。
2. **強制技能**：使用 `Skill tool` → `frontend` 載入前端開發規範。若涉及 UI 設計，額外載入 `Skill tool` → `frontend-design`。
3. **輸入**：提供來自階段 A 的 **已驗證 API 契約**。
4. **任務**：
    - 實作前端介面。
    - 遵循 `frontend` Skill 中的 RWD 回應式設計與組件指南。
...
1. **角色**：QA 工程師。
2. **強制技能**：使用 `Skill tool` → `qa` 載入 QA 測試規範。
...
2. **如果測試失敗 (FAIL)**：
    ...
    - **指派修復**：
        ...
        - 前端問題 → 重新指派 **Frontend Agent** 修復（須重新載入 `frontend` Skill）。
